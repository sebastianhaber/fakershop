...
export default function ProductPage() {
  ...
  const { products, cart, setCart, favouriteProducts, setFavouriteProducts } =
    ...
    <StyledProduct>
      ...
      <div className="flex">
        ...
        <div className="item-info">
          ...
          {product.qty && (
            <div className="qty-wrapper">{`Qty: <b>${product.qty}</b>`}</div>
          )}
          <span className="priceWrapper">
            Price: <p className="price underline">${product.price}</p>
          </span>
          <div className="buttons_wrapper">
            <div className="qty">
              <h4>Quantity</h4>
              <div className="qty_wrapper">
                <button onClick={decreaseQty} disabled={selectedQty < 2}>
                  -
                </button>
                <p>{selectedQty}</p>
            ...
            <div className="buttons">
              <div
                ...
                className={isFavourite ? `favourite isFavourite` : `favourite`}
                onClick={() => handleFavourite(product)}
              >
                <Icon icon="akar-icons:heart" />
              </div>
              <button className="full" onClick={handeAddToCart}>
                Add to cart{" "}
                {`($${(selectedQty * Number.parseFloat(product.price)).toFixed(
                  2
                )})`}
              </button>
            </div>
